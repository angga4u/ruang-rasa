...
if ($method === 'GET') {
    ...
    if ($id) {
        ...
        $sql    = "SELECT c.*, (SELECT COUNT(*) FROM likes l WHERE l.cerita_id = c.id) as likes
                   FROM cerita c WHERE c.privasi = 'publik'";
        $params = [];
        $types  = '';
...
// ===== POST =====
    ...
    $perasaan       = isset($body['perasaan'])       ? trim($body['perasaan'])       : 'sedih';
    $privasi        = isset($body['privasi'])        ? trim($body['privasi'])        : 'publik';
    $user_id        = isset($body['user_id'])        ? intval($body['user_id'])      : 1;

    if (!in_array($privasi, ['publik', 'anonim'])) $privasi = 'publik';
    ...
    $stmt = $db->prepare(
        "INSERT INTO cerita (user_id, judul, isi, nama_panggilan, perasaan, privasi, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'diterbitkan', NOW(), NOW())"
    );
    $stmt->bind_param('isssss', $user_id, $judul, $isi, $nama_panggilan, $perasaan, $privasi);
